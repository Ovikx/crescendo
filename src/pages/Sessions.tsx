...
export const Sessions = ({ navigation }: Props) => {
  ...
  for (const session of sessions) {
    ...
    const fmttedDate = `${
      months[sessionDate.getMonth() as keyof typeof months]
      ...
      <View className="flex-1 w-11/12 mx-auto">
        <SectionList
          ...
          renderItem={({ item }) => {
            return (
              <View className="py-1">
                <SessionCard name={item.itemName} timeSpent={item.seconds} />
              </View>
            );
          }}
          renderSectionHeader={({ section }) => {
            return <SessionSectionHeader title={section.title} />;
          }}
        />
      </View>
